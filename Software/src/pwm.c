...
static s_channel_info channel_info_a[PWM_LAST_CHANNEL];
static s_channel_info channel_info_b[PWM_LAST_CHANNEL];
...
void pwm_init(void)
{
    ...
    pwm_all_bitmask = 0;
    // initialise
    for (i = PWM_FIRST_CHANNEL - 1; i < PWM_LAST_CHANNEL; i++)
    {
...
static void add_channel(uint16_t pulse_ticks, uint8_t ch)
{
    ...
    if (i_free_chan >= PWM_LAST_CHANNEL)
        return;
