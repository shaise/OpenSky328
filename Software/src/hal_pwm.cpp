...
// set compare match register for 20ms interval
  ...
  DDRD |= PORTD_PWM_BITS;   // portd pwm pins set to output
  PORTD &= ~PORTD_PWM_BITS; // start with bits set to 0
  DDRB |= PORTB_PWM_BITS;   // portb pwm pins set to output
  PORTB &= ~PORTB_PWM_BITS; // start with bits set to 0
